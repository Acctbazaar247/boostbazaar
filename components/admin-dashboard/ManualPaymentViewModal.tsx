...
const ManualPaymentViewModal = ({ data }: Props) => {
  const [receivedAmount, setReceivedAmount] = useState<number>(
    data.receivedAmount || data.requestedAmount || 0,
  );
  const [updateManualCurrencyRequest, { isLoading }] =
    ...
    <div>
      <div>
        ...
        <AppInput
          ...
          setValue={e => setReceivedAmount(Number(e.target.value))}
        ></AppInput>
        ...
        <div className="py-3 text-center">
          ...
          {data.bankId ? (
            <div>
              <p>
                The Dollar Rate is{' '}
                <span className="text-orange-500">{data.dollarRate}</span>{' '}
                <br /> The amount they have to pay in naira is{' '}
                <span className="text-orange-500">
                  {' '}
                  {data.requestedAmount * data.dollarRate}
                </span>
              </p>
              ...
              <div className="text-center">
                <h2 className="text-lg font-bold">Admin Bank info</h2>
              </div>
              <div className="grid grid-cols-2 gap-2">
                <div className="flex items-center border border-borderColor rounded-md p-2 mt-2 flex-col gap-2">
                  <h2>Bank Name</h2>
