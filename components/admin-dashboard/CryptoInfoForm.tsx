...
const CryptoInfoForm = (props: Props) => {
  ...
  const [addCryptoBank, { isLoading: addCryptoBankLoading }] =
    useAddCryptoBankMutation();
  const [editCryptoBank, { isLoading: editCryptoBankLoading }] =
    useEditCryptoBankMutation();
  const onSubmit = (data: any) => {
    if (props.id) {
      editCryptoBank({ ...data, id: props.id })
        .unwrap()
        .then(() => {
  ...
  return (
    <div className="lg:w-[700px] py-4">
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <button
          type="submit"
          disabled={addCryptoBankLoading}
          className="bg-green-600  mt-5 text-white px-3 py-2  rounded-md flex items-center gap-2 disabled:opacity-55 transition-all"
        >
          {' '}
          {props.id ? 'Update Crypto Bank' : 'Add Crypto Bank'}
        </button>
      </form>
    </div>
  );
