...
const MyNumberDetails = ({ id, isAdmin }: Props) => {
  const { isLoading, data, isError, refetch } = useGetSmsPoolByIdQuery(id, {});
          ...
          {mainData.details.status !== mainData.info.status && (
            <div className="mt-4">
              ...
              <p className="text-orange-700">
                ...
                status on sms pool is ({mainData.details.status}) and on our own
                data base its is ({mainData.info.status})
              </p>
              <button
