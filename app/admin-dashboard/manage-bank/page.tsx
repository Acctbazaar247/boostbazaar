...
const ManageBank = (props: Props) => {
  ...
  const [deleteBank, { isLoading: isDeleteBankLoading }] =
    useDeleteBankMutation();
  const columns = [
    {
      title: 'Bank Name',
      ...
      render: (bankName: string, record: any) => {
        return <p className="line-clamp-1  text-base">{bankName}</p>;
      },
    },
    {
      ...
      render: (accountName: string, record: any) => {
        return <p className="line-clamp-1  text-base">{accountName}</p>;
      },
    },
    {
      ...
      render: (accountNumber: string, record: any) => {
        return <p className="line-clamp-1  text-base">{accountNumber}</p>;
      },
    },
    // status
    {
        ...
        return (
          <div className="flex items-center gap-2">
            <AppSwitch
              checked={isActive}
              onChange={checked => {
                editBank({ id: record.id, isActive: checked })
                  .unwrap()
                  .then(() => {
