...
const ManageCryptoBank = (props: Props) => {
  ...
  const [editCryptoBank, { isLoading: editCryptoBankLoading }] =
    useEditCryptoBankMutation();
  const [deleteCryptoBank, { isLoading: deleteCryptoBankLoading }] =
    useDeleteCryptoBankMutation();
  const columns = [
    {
        ...
        return (
          <div className="flex items-center gap-2">
            <AppSwitch
              checked={isActive}
              onChange={checked => {
                editCryptoBank({ id: record.id, isActive: checked })
                  .unwrap()
                  .then(() => {
                    toast.success('Crypto Bank status updated successfully', {
                      toastId: '1',
                    });
                  })
                  .catch(error => {
        ...
        return (
          <div className="flex items-center gap-2">
            <AppModal
              ...
              button={
                <button className="bg-orange-600 text-white px-2 py-1 rounded-md flex items-center gap-2">
                  {' '}
                  <FaEdit /> Edit
                </button>
              }
            ...
            <button
              disabled={deleteCryptoBankLoading}
              onClick={() => {
                deleteCryptoBank(record.id)
                  .unwrap()
                  .then(() => {
                    toast.success('Crypto Bank deleted successfully', {
                      toastId: '1',
                    });
                  })
                  .catch(error => {
              ...
              className="bg-red-600 text-textBlack px-2 py-1 rounded-md flex items-center gap-2"
            >
              {' '}
              <FaTrash /> Delete
            </button>
    ...
    <div>
      <div className="flex justify-between items-center">
        ...
        <AppModal
          ...
          button={
            <button className="bg-green-600 text-white px-2 py-1 rounded-md flex items-center gap-2">
              {' '}
              <FaPlus /> Add Crypto Bank
            </button>
          }
        >
          <div>
